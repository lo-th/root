...
SEA3D.File.prototype.readState=function(){for(;this.state(););this.state?(requestAnimationFrame(this.readState.bind(this)),this.dispatchProgress()):(this.dispatchProgress(),this.dispatchComplete())};SEA3D.File.prototype.read=function(a){if(!a)throw Error("No data found.");this.initParse();this.stream=new SEA3D.Stream(a);this.state=this.readHead;this.readState()};SEA3D.File.prototype.dispatchCompleteObject=function(a){if(this.onCompleteObject)this.onCompleteObject({file:this,object:a})};
...
Object.assign(THREE.SEA3D.Animator.prototype,{update:function(a){this.mixer.update(a||0);if(this.currentAnimationAction&&this.currentAnimationAction.paused&&(this.pause(),this.currentAnimationData&&this.currentAnimationData.onComplete))this.currentAnimationData.onComplete(this);return this},updateAnimations:function(a){this.playing&&this.stop();this.mixer&&THREE.SEA3D.AnimationHandler.removeAnimator(this);THREE.SEA3D.AnimationHandler.addAnimator(this);this.mixer=a;this.playing=this.relative=!1;this.timeScale=
1;this.animId=[];this.animations={};this.animationsData={};if(!this.geometry||void 0!==this.geometry.animations){this.clips=this instanceof THREE.SEA3D.Animator?this.clips:this.geometry.animations;a=0;for(var b=this.clips;a<b.length;a++)this.addAnimation(b[a])}},addAnimation:function(a){var b=a.name;this.animations[b]=a;this.animationsData[b]={};this.animId.push(b)},removeAnimation:function(a){delete this.animations[a.name];this.mixer.uncacheClip(a)},playw:function(a,b){var c=this.animations[a],d=
this.mixer.clipAction(c);this.playing=!0;d.setLoop(c.loop?THREE.LoopRepeat:THREE.LoopOnce,Infinity).reset();d.clampWhenFinished=!c.loop;d.paused=!1;d.setEffectiveWeight(b);return d.play()},crossfade:function(a,b,c){this.mixer.stopAllAction();a=this.playw(a,1);b=this.playw(b,1);a.crossFadeTo(b,c,!1)},warp:function(a,b,c){this.mixer.stopAllAction();a=this.playw(a,1);b=this.playw(b,1);a.crossFadeTo(b,c,!0)},stopAll:function(){this.mixer.stopAllAction()},unPauseAll:function(){this.mixer.timeScale=1},
pauseAll:function(){this.mixer.timeScale=0},setClipTimeScale:function(a,b){this.mixer.clipAction(this.animations[a]).setEffectiveTimeScale(b)},getStateByName:function(a){return this.animations.indexOf(this.animations[a])},getStateNameByIndex:function(a){return this.animations[this.animId[a]].name},pause:function(){this.playing&&this.currentAnimation&&(THREE.SEA3D.AnimationHandler.removeAnimator(this),this.playing=!1)},resume:function(){!this.playing&&this.currentAnimation&&(THREE.SEA3D.AnimationHandler.addAnimator(this),
this.playing=!0);return this},setTimeScale:function(a){this.timeScale=a;this.currentAnimationAction&&this.updateTimeScale()},getTimeScale:function(){return this.timeScale},updateTimeScale:function(){this.currentAnimationAction.setEffectiveTimeScale(this.timeScale*(this.currentAnimation?this.currentAnimation.timeScale:1))},play:function(a,b,c,d){var e=isNaN(a)?this.animations[a]:this.animations[this.animId[a]];if(e==this.currentAnimation)return void 0===c&&e.loop||(this.currentAnimationAction.time=
...
THREE.SEA3D.Mesh.prototype=Object.assign(Object.create(THREE.Mesh.prototype),THREE.SEA3D.Mesh.prototype,THREE.SEA3D.Object3D.prototype,{constructor:THREE.SEA3D.Mesh,setWeight:function(a,b){this.animations&&this.animations[a]&&this.mixer.clipAction(a).setEffectiveWeight(b);this.morphTargetInfluences&&void 0!==this.morphTargetDictionary[a]&&(this.morphTargetInfluences[this.morphTargetDictionary[a]]=b)},getWeight:function(a){if(this.animations&&this.animations[a])return this.mixer.clipAction(a).getEffectiveWeight();
if(this.morphTargetDictionary&&void 0!==this.morphTargetDictionary[a])return this.morphTargetInfluences[this.morphTargetDictionary[a]]},copy:function(a){THREE.Mesh.prototype.copy.call(this,a);this.attribs=a.attribs;this.scripts=a.scripts;this.animator&&(this.animator=a.animator.clone(this));return this}});THREE.SEA3D.SkinnedMesh=function(a,b,c){THREE.SkinnedMesh.call(this,a,b,c);this.updateAnimations(new THREE.AnimationMixer(this))};
...
THREE.SEA3D.prototype.readAnimation=function(a){for(var b=[],c=1E3/a.frameRate/1E3,d=0;d<a.sequence.length;d++){for(var e=a.sequence[d],f=[],h=0;h<a.dataList.length;h++){var g=a.dataList[h],k,m,l,q=g.data;l=e.start*g.blockSize;k=l+e.count*g.blockSize;var r=e.intrpl?THREE.InterpolateLinear:!1,s=null;switch(g.kind){case SEA3D.Animation.POSITION:s=".position";break;case SEA3D.Animation.ROTATION:s=".quaternion";break;case SEA3D.Animation.SCALE:s=".scale";break;case SEA3D.Animation.COLOR:s=".color";break;
case SEA3D.Animation.MULTIPLIER:s=".intensity";break;case SEA3D.Animation.FOV:s=".fov"}if(s)switch(g.type){case SEA3D.Stream.BYTE:case SEA3D.Stream.UBYTE:case SEA3D.Stream.INT:case SEA3D.Stream.UINT:case SEA3D.Stream.FLOAT:case SEA3D.Stream.DOUBLE:case SEA3D.Stream.DECIMAL:l=q.subarray(l,k);m=new Float32Array(l.length);for(g=k=0;g<m.length;g++)m[g]=k,k+=c;f.push(new THREE.VectorKeyframeTrack(s,m,l,r));break;case SEA3D.Stream.VECTOR3D:l=q.subarray(l,k);m=new Float32Array(l.length/g.blockSize);for(g=
k=0;g<m.length;g++)m[g]=k,k+=c;f.push(new THREE.VectorKeyframeTrack(s,m,l,r));break;case SEA3D.Stream.VECTOR4D:l=q.subarray(l,k);m=new Float32Array(l.length/g.blockSize);for(g=k=0;g<m.length;g++)m[g]=k,k+=c;f.push(new THREE.QuaternionKeyframeTrack(s,m,l,r));break;case SEA3D.Stream.INT24:case SEA3D.Stream.UINT24:l=new Float32Array(3*(k-l));m=new Float32Array(l.length/3);for(g=k=0;g<m.length;g++)l[3*g]=(q[g]>>16&255)/255,l[3*g+1]=(q[g]>>8&255)/255,l[3*g+2]=(q[g]&255)/255,m[g]=k,k+=c;f.push(new THREE.VectorKeyframeTrack(s,
m,l,r))}}f=new THREE.AnimationClip(e.name,-1,f);f.loop=e.repeat;f.timeScale=1;b.push(f)}this.domain.animationClips=this.animationClips=this.animationClips||[];this.animationClips.push(this.objects[a.name+".#anm"]=a.tag=b)};
...
THREE.SEA3D.prototype.readDummy=function(a){var b=new THREE.SEA3D.Dummy(a.width,a.height,a.depth);b.name=a.name;this.domain.dummys=this.dummys=this.dummys||[];this.dummys.push(this.objects["dmy/"+a.name]=a.tag=b);this.addSceneObject(a);this.updateTransform(b,a);this.applyDefaultAnimation(a,THREE.SEA3D.Object3DAnimator)};
THREE.SEA3D.prototype.readLine=function(a){var b=new THREE.BufferGeometry;a.closed&&a.vertex.push(a.vertex[0],a.vertex[1],a.vertex[2]);THREE.Float32BufferAttribute?b.addAttribute("position",new THREE.Float32BufferAttribute(a.vertex,3)):b.addAttribute("position",new THREE.BufferAttribute(new Float32Array(a.vertex),3));b=new THREE.Line(b,new THREE.LineBasicMaterial({color:THREE.SEA3D.HELPER_COLOR,linewidth:3}));b.name=a.name;this.lines=this.lines||[];this.lines.push(this.objects["line/"+a.name]=a.tag=
b);this.addSceneObject(a);this.updateTransform(b,a);this.applyDefaultAnimation(a,THREE.SEA3D.Object3DAnimator)};THREE.SEA3D.prototype.readContainer3D=function(a){var b=new THREE.SEA3D.Object3D;this.domain.containers=this.containers=this.containers||[];this.containers.push(this.objects["c3d/"+a.name]=a.tag=b);this.addSceneObject(a);this.updateTransform(b,a);this.applyDefaultAnimation(a,THREE.SEA3D.Object3DAnimator)};
...
THREE.SEA3D.prototype.readMorpher=function(a){var b={position:[]},c=[];if(THREE.Float32BufferAttribute)for(var d=0;d<a.node.length;d++){var e=a.node[d];b.position[d]=new THREE.Float32BufferAttribute(e.vertex,3);e.normal&&(b.normal=b.normal||[],b.normal[d]=new THREE.Float32BufferAttribute(e.normal,3));c[d]={name:e.name}}else for(d=0;d<a.node.length;d++)e=a.node[d],b.position[d]=new THREE.Float32Attribute(new Float32Array(e.vertex),3),e.normal&&(b.normal=b.normal||[],b.normal[d]=new THREE.Float32Attribute(new Float32Array(e.normal),
3)),c[d]={name:e.name};a.tag={attribs:b,targets:c}};
THREE.SEA3D.prototype.getSkeletonAnimation=function(a,b){if(a.tag)return a.tag;for(var c=[],d=1E3/a.frameRate/1E3,e=0;e<a.sequence.length;e++){for(var f=a.sequence[e],h=f.start,g=h+f.count,k={name:f.name,fps:a.frameRate,length:d*f.count,hierarchy:[]},m=a.numJoints,l=a.raw,q=0;q<m;q++){for(var r={parent:b.joint[q].parentIndex,keys:[]},s=r.keys,t=0,p=h;p<g;p++){var n=p*m*7+7*q;s.push({time:t,pos:[l[n],l[n+1],l[n+2]],rot:[l[n+3],l[n+4],l[n+5],l[n+6]],scl:[1,1,1]});t+=d}k.hierarchy[q]=r}h=THREE.AnimationClip.parseAnimation(k,
b.tag);h.loop=f.repeat;h.timeScale=1;c.push(h)}return a.tag=c};
THREE.SEA3D.prototype.readVertexAnimation=function(a){var b={position:[]},c=[],d=[],e,f,h;if(THREE.Float32BufferAttribute)for(e=0,h=a.frame.length;e<h;e++)f=a.frame[e],b.position[e]=new THREE.Float32BufferAttribute(f.vertex,3),f.normal&&(b.normal=b.normal||[],b.normal[e]=new THREE.Float32BufferAttribute(f.normal,3)),c[e]={name:e};else for(e=0,h=a.frame.length;e<h;e++)f=a.frame[e],b.position[e]=new THREE.Float32Attribute(new Float32Array(f.vertex),3),f.normal&&(b.normal=b.normal||[],b.normal[e]=new THREE.Float32Attribute(new Float32Array(f.normal),
3)),c[e]={name:e};for(e=0;e<a.sequence.length;e++){h=a.sequence[e];var g=[];for(f=0;f<h.count;f++)g[f]=c[h.start+f];f=THREE.AnimationClip.CreateFromMorphTargetSequence(h.name,g,a.frameRate);f.loop=h.repeat;f.timeScale=1;d.push(f)}a.tag={attribs:b,targets:c,animations:d}};
...
THREE.SEA3D.prototype.updateAnimationSet=function(a){var b=new THREE.Matrix4,c=new THREE.Matrix4,d=new THREE.Vector3,e=new THREE.Quaternion,f=new THREE.Vector3,h=new THREE.Vector3,g=new THREE.Quaternion,k=new THREE.Vector3;return function(a){var l=a.animation.animationSet,q=l.animations;if(!l.flip||q.length){for(var r=q[0].dataList,q=[],s=0;s<r.length;s++){var t=r[s],p=r[s].data,n=t.kind,t=p.length/t.blockLength;switch(n){case SEA3D.Animation.POSITION:case SEA3D.Animation.ROTATION:case SEA3D.Animation.SCALE:q.push({kind:n,
numFrames:t,raw:p})}}if(0<q.length)for(t=q[0].numFrames,p=void 0,a.animation.relative?(b.identity(),this.flipMatrixScale(c.copy(a.matrixWorld))):(a.parent&&(p=this.flipMatrixScale(c.copy(a.parent.matrixWorld))),this.flipMatrixScale(b.copy(a.matrix),!1,p)),b.decompose(d,e,f),a=0;a<t;a++){for(r=0;r<q.length;r++)switch(p=q[r].raw,n=q[r].kind,n){case SEA3D.Animation.POSITION:n=3*a;d.set(p[n],p[n+1],p[n+2]);break;case SEA3D.Animation.ROTATION:n=4*a;e.set(p[n],p[n+1],p[n+2],p[n+3]);break;case SEA3D.Animation.SCALE:n=
4*a,f.set(p[n],p[n+1],p[n+2])}b.compose(d,e,f);this.flipMatrixScale(b,!1,c);b.decompose(h,g,k);for(r=0;r<q.length;r++)switch(p=q[r].raw,n=q[r].kind,n){case SEA3D.Animation.POSITION:n=3*a;p[n]=h.x;p[n+1]=h.y;p[n+2]=h.z;break;case SEA3D.Animation.ROTATION:n=4*a;p[n]=g.x;p[n+1]=g.y;p[n+2]=g.z;p[n+3]=g.w;break;case SEA3D.Animation.SCALE:n=3*a,p[n]=k.x,p[n+1]=k.y,p[n+2]=k.z}}l.flip=!0}}}();
...
THREE.SEA3D.prototype.readSkeleton=function(a){var b=new THREE.Matrix4,c=new THREE.Matrix4,d=new THREE.Matrix4,e=new THREE.Vector3,f=new THREE.Quaternion;return function(a){for(var g=[],k=a.sea3d.config.legacy,m=0;m<a.joint.length;m++){var l=a.joint[m];b.elements=l.inverseBindMatrix;c.getInverse(b);k&&this.flipMatrixBone(c);-1<l.parentIndex&&(b.elements=a.joint[l.parentIndex].inverseBindMatrix,d.getInverse(b),k&&this.flipMatrixBone(d),d.getInverse(d),c.multiplyMatrices(d,c));e.setFromMatrixPosition(c);
f.setFromRotationMatrix(c);g[m]={name:l.name,pos:[e.x,e.y,e.z],rotq:[f.x,f.y,f.z,f.w],parent:l.parentIndex}}return a.tag=g}}();THREE.SEA3D.prototype.getSkeletonAnimation=function(a,b){return this.isLegacy(a)?this.getSkeletonAnimationLegacy(a,b):this._getSkeletonAnimation(a,b)};
THREE.SEA3D.prototype.getSkeletonAnimationLegacy=function(a,b){var c=new THREE.Matrix4,d=new THREE.Matrix4,e=new THREE.Matrix4,f=new THREE.Matrix4;return function(a,b){if(a.tag)return a.tag;for(var k=[],m=1E3/a.frameRate/1E3,l=[1,1,1],q=0;q<a.sequence.length;q++){for(var r=a.sequence[q],s=r.start,t=s+r.count,p={name:r.name,repeat:r.repeat,fps:a.frameRate,JIT:0,length:m*r.count,hierarchy:[]},n=a.numJoints,v=a.raw,w=0;w<n;w++){for(var y=b.joint[w],B={parent:y.parentIndex,keys:[]},C=B.keys,z=0,A=s;A<
...
u.y,u.z],rot:[x.x,x.y,x.z,x.w],scl:l});z+=m}p.hierarchy[w]=B}s=THREE.AnimationClip.parseAnimation(p,b.tag);s.loop=r.repeat;s.timeScale=1;k.push(s)}return a.tag=k}}();THREE.SEA3D.prototype.readVertexAnimation=function(a){if(this.isLegacy(a))for(var b=0,c=a.frame.length;b<c;b++){var d=a.frame[b];this.flipZVec3(d.vertex);this.flipZVec3(d.normal)}this._readVertexAnimation(a)};
