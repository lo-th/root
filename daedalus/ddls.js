...
DDLS.int = function(x) { return parseInt(x); };
...
DDLS.Object.prototype = {
    ...
    updateMatrixFromValues: function() {
        this._matrix.identity().translate(-this._pivotX,-this._pivotY).scale(this._scaleX,this._scaleY).rotate(this._rotation).translate(this._x,this._y);
...
DDLS.Geom2D.pathLength = function(path) {
    ...
    while(i < path.length) {
        ...
        x = nextX - fromX;
        y = nextY - fromY;
        distance = DDLS.sqrt(DDLS.Squared(x, y));
    ...
    triangulate: function(bound,isReal) {
        if(bound.length < 2) {
           ...
           // DDLS.Debug.trace("  - edge1: " + bound[1].originVertex.id + " -> " + bound[1].destinationVertex.id,{ fileName : "Mesh.hx", lineNumber : 1405, className : "DDLS.Mesh", methodName : "triangulate"});
            ...
            var boundM;
            if(index == 2) boundM = [baseEdge,bound[1],edgeAopp]; 
            else if(index == bound.length - 1) boundM = [baseEdge,edgeB,bound[index]]; 
            else boundM = [baseEdge,edgeB,edgeAopp];
...
DDLS.Potrace.buildPolygon = function(graph,debugShape) {
    ...
    if(DDLS.Geom2D.getDirection(new DDLS.Point(polygon[polygon.length - 2],polygon[polygon.length - 1]),new DDLS.Point(polygon[0],polygon[1]),new DDLS.Point(polygon[2],polygon[3])) == 0) {
...
DDLS.AStar.prototype = {
    ...
    findPath: function(fromX,fromY,toX,toY,resultListFaces,resultListEdges) {
        ...
        loc = DDLS.Geom2D.locatePosition(new DDLS.Point(fromX, fromY), this.mesh);
        if ( loc.type == 0 ){
        ...
        //
        loc = DDLS.Geom2D.locatePosition(new DDLS.Point(toX, toY), this.mesh);
        if ( loc.type == 0 ){
            locVertex = loc;
        ...
        //0;
        var dist = DDLS.sqrt(DDLS.Squared(toX - fromX, toY - fromY));
        ...
        while(true) {
            ...
            //this.sortedOpenedFaces.sort(this.sortingFaces());
        }
...
DDLS.Funnel.prototype = {
    ...
    findPath: function(fromX,fromY,toX,toY,listFaces,listEdges,resultPath) {
        var p_from = new DDLS.Point(fromX,fromY);
        var p_to = new DDLS.Point(toX,toY);
...
DDLS.PathFinder.prototype = {
    ...
    findPath: function(toX,toY,resultPath) {
        ...
        if(DDLS.Geom2D.isCircleIntersectingAnyConstraint(new DDLS.Point(toX,toY),this.entity.radius,this._mesh)) return;
...
DDLS.FieldOfView.prototype = {
    isInField:function(targetEntity){
        ...
        var pos =  new DDLS.Point(this._fromEntity.x, this._fromEntity.y);
        var direction = new DDLS.Point(this._fromEntity.dirNormX, this._fromEntity.dirNormY);
        var target = new DDLS.Point(targetEntity.x, targetEntity.y);
        ...
        if ( DDLS.Geom2D.intersections2Circles(pos, radius, target, targetRadius, result) ){
            leftTarget.x = result[0];
            leftTarget.y = result[1];
            rightTarget.x = result[2];
            rightTarget.y = result[3];
        }
        ...
        var dotProdMin = DDLS.cos(this._fromEntity.angleFOV*0.5);
        ...
        var startObj = DDLS.Geom2D.locatePosition(pos, this._mesh);
        var startFace;
...
DDLS.RectMesh = {};
DDLS.RectMesh = function(w,h, oldMesh) {
    var old = oldMesh || null;
    var v = [];
    var e = [];
    var f = [];
    var s = [];
    ...
    var mesh;
    if(old==null) mesh = new DDLS.Mesh(w,h);
    else mesh = old;
    mesh._vertices = v;
    mesh._edges = e;
    mesh._faces = f;
...
DDLS.SimpleView.prototype = {
    ...
    drawEntity: function(entity,cleanBefore) {
        ...
        this.g.beginFill(this.entitiesColor);
...
DDLS.ImageLoader = function(imageNames,loaded_) {
    this.images = new DDLS.Dictionary(2);//DDLS.StringMap();
    this.loaded = loaded_;
