...
void main() {
   ...
   //vec3 N = normalize( mat3( matrix[0].xyz, matrix[1].xyz, matrix[2].xyz ) * texture2D(normalMap, vUv).xyz );//texture2D(normalMap, vUv).xyz;
    ...
    float fct = mExtraPower;

    finalColor.rgb = vec3( pow( abs(finalColor.r), fct), pow(abs(finalColor.g), fct), pow(abs(finalColor.b), fct) );
