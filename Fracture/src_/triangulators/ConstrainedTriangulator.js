...
import { linesIntersect, hash2 } from "../utils/MathUtils.js";
...
export class ConstrainedTriangulator extends Triangulator {
     ...
     */
    constructor(inputPoints, constraints, normal) {
     ...
     */
    findIntersectingEdges(constraint, vertexTriangles) {
     ...
     * @returns
     */
    findStartingEdge(vertexTriangles, constraint) {
        ...
        let intersectingEdgeIndex = null
        let tE12, tE23, tE31
        const visited = new Array(this.triangulation.length)
        while (!intersectingEdgeIndex && !noCandidatesFound) {
            ...
            if ( tE12 !== OUT_OF_BOUNDS && !visited[tE12] && this.triangleContainsVertex(tE12, v_i) ) {
                tSearch = tE12
            } else if ( tE23 !== OUT_OF_BOUNDS && !visited[tE23] && this.triangleContainsVertex(tE23, v_i) ) {
                tSearch = tE23
            } else if ( tE31 !== OUT_OF_BOUNDS && !visited[tE31] && this.triangleContainsVertex(tE31, v_i) ) {
                tSearch = tE31
            } else {
    ...
    discardTrianglesViolatingConstraints() {
        ...
        let v1, v2, v3
        let boundaryE12, boundaryE23, boundaryE31
        const visited = new Array(this.triangulation.length)
        for (let i = 0; i < this.triangleCount; i++) {
    ...
    triangleContainsConstraint(t, constraint) {
     ...
     */
    edgeConstraintIntersectsTriangle(t, constraint) {
        const v_i = this.points[constraint.v1].coords
        const v_j = this.points[constraint.v2].coords
        const v1 = this.points[this.triangulation[t][V1]].coords
        const v2 = this.points[this.triangulation[t][V2]].coords
     ...
     */
    findQuadFromSharedEdge(t1, t1SharedEdge) {
        //               q3
        //      *---------*---------*
        //       \       / \       /
        //        \ t2L /   \ t2R /
     ...
     */
    swapQuadDiagonal(quad, edges1, edges2, edges3) {
        // BEFORE
        //               q3
        //      *---------*---------*
        //       \       / \       /
     ...
     * Update the edges
     */
    updateEdgesAfterSwap( edges, t1, t2, t1L, t1R, t2L, t2R ) {

        if (!edges) return

        //if ( edges===undefined || edges===null ) return null;
