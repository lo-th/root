...
import { Fragment, SlicedMeshSubmesh } from "./entities/Fragment.js";
import { isPointAbovePlane, linePlaneIntersection } from "./utils/MathUtils.js";
import MeshVertex from "./entities/MeshVertex.js";
import EdgeConstraint from "./entities/EdgeConstraint.js";
import { Triangulator } from "./triangulators/Triangulator.js";
 ...
 */
export function Slice( fragment, sliceNormal, sliceOrigin, textureScale, textureOffset, convex ) {
    ...
    const side = new Array(fragment.vertexCount).fill(false);
    ...
    fillCutFaces(topSlice, bottomSlice, sliceNormal, textureScale, textureOffset, convex);
 ...
 * @returns
 */
function fillCutFaces( topSlice, bottomSlice, sliceNormal, textureScale, textureOffset, convex ) {
    ...
    const inversNormal = sliceNormal.clone().negate()//.normalize()

    //console.log(sliceNormal, inversNormal)
    ...
    //const triangulator = convex ? new Triangulator(topSlice.cutVertices, sliceNormal) : new ConstrainedTriangulator(topSlice.cutVertices, topSlice.constraints, sliceNormal);
    const triangulator = convex ? new Triangulator(topSlice.cutVertices, inversNormal) : new ConstrainedTriangulator(topSlice.cutVertices, topSlice.constraints, inversNormal);


    const triangles = triangulator.triangulate();

    //console.log('Num triangles:', triangles.length/3)
    ...
    for (let i = 0; i < topSlice.cutVertices.length; i++) {
        ...
        //const topVertex = new MeshVertex(vertex.position.clone(), sliceNormal.clone(), uv.clone());
        //const bottomVertex = new MeshVertex(vertex.position.clone(), sliceNormal.clone().negate(), uv.clone());

        const topVertex = new MeshVertex(vertex.position.clone(), inversNormal, uv.clone());
        const bottomVertex = new MeshVertex(vertex.position.clone(), sliceNormal, uv.clone());
    ...
    for (let i = 0; i < triangles.length; i += 3) {
        topSlice.addTriangle(offsetTop + triangles[i], offsetTop + triangles[i + 1], offsetTop + triangles[i + 2], SlicedMeshSubmesh.CutFace);
        bottomSlice.addTriangle(offsetBottom + triangles[i], offsetBottom + triangles[i + 2], offsetBottom + triangles[i + 1], SlicedMeshSubmesh.CutFace);
    }
}
 ...
 */
function splitTriangles(fragment, topSlice, bottomSlice, sliceNormal, sliceOrigin, side, subMesh) {
    

    const triangles = fragment.triangles[subMesh];
 ...
 */
function splitTriangle(v1_idx, v2_idx, v3_idx, sliceNormal, sliceOrigin, fragment, topSlice, bottomSlice, subMesh, v3BelowCutPlane) {
    ...
    let v1 = v1_idx < fragment.vertices.length ? fragment.vertices[v1_idx] : fragment.cutVertices[v1_idx - fragment.vertices.length];

    let v2 = v2_idx < fragment.vertices.length ? fragment.vertices[v2_idx] : fragment.cutVertices[v2_idx - fragment.vertices.length];

    let v3 = v3_idx < fragment.vertices.length ? fragment.vertices[v3_idx] : fragment.cutVertices[v3_idx - fragment.vertices.length];
    ...
    if (v13 && v23) {
        ...
        const index13_A = topSlice.vertices.length - 2;
        const index23_A = topSlice.vertices.length - 1;
        const index13_B = bottomSlice.vertices.length - 2;
        const index23_B = bottomSlice.vertices.length - 1;
        ...
        if (v3BelowCutPlane) {
            ...
            topSlice.constraints.push(new EdgeConstraint(topSlice.cutVertices.length - 2, topSlice.cutVertices.length - 1));
            bottomSlice.constraints.push(new EdgeConstraint(bottomSlice.cutVertices.length - 1, bottomSlice.cutVertices.length - 2));
        } else {
            ...
            // When looking at the cut-face, the edges should wind counter-clockwise
            topSlice.constraints.push(new EdgeConstraint(topSlice.cutVertices.length - 1, topSlice.cutVertices.length - 2));
            bottomSlice.constraints.push(new EdgeConstraint(bottomSlice.cutVertices.length - 2, bottomSlice.cutVertices.length - 1));
        }
    }
}