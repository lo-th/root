...
class RollerCoasterGeometry extends BufferGeometry {
	...
	constructor( curve, divisions, type ) {
		...
		for ( let i = 0; i < sides; i ++ ) {
			...
			const angle = P22 + ( i / sides ) * PI2;
			tube2.push( new Vector3( Math.sin( angle ) * size, Math.cos( angle ) * size, 0 ) );
		...
		const center = [
		    new Vector3(-0.21, 0, 0), new Vector3(0, -0.0001, 0), new Vector3(0.21, 0, 0),
		    //new Vector3(-0.2, -0.1, 0),  new Vector3(0, -0.1, 0), //new Vector3(0.2, -0.1, 0)
		...
		function simpleExtrudeShape( shape, offset, color ) {

			for ( let j = 0, jl = shape.length-1; j < jl; j ++ ) {

				const point1 = shape[ j ];
				const point2 = shape[ ( j + 1 )  ];
		...
		for ( let i = 1; i <= divisions; i ++ ) {
			...
			if(type) simpleExtrudeShape( center, offset.set( 0, 0, 0 ), color2 );
			else {
				extrudeShape( tube2, offset.set( 0.2+size, 0, 0 ), color1 );
				extrudeShape( tube2, offset.set( - 0.2-size, 0, 0 ), color1 );
			}
