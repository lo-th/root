...
this.num;a--;)this.members[a].update()},postUpdate:function(){for(var a=this.num;a--;)this.members[a].postUpdate()},preDraw:function(){for(var a=this.num;a--;)this.members[a].visible&&this.members[a].preDraw()},draw:function(){for(var a=this.num;a--;)this.members[a].visible&&this.members[a].draw()},debugDraw:function(){for(var a=this.num;a--;)this.members[a].debugDraw()}};FAST.World=function(){FAST.Group.call(this);this.islands=[]};FAST.World.prototype=Object.create(FAST.Group.prototype);FAST.World.prototype.preUpdate=function(){for(var a=this.islands.length;a--;)this.islands[a].solve();for(a=this.num;a--;)this.members[a].active&&this.members[a].preUpdate()};FAST.World.prototype.addIsland=function(a){this.islands[this.islands.length]=a};
FAST.Simulation=function(a,b,c,d){this.requestedWorld=a;this.currentWorld=null;this.currentWorldInstance=a;FAST.frameRate=b||60;FAST.width=c||600;FAST.height=d||480;this.total=0};
FAST.Simulation.prototype={constructor:FAST.Simulation,update:function(){var a=Date.now(),b=(a-this.total)/1E3;this.total=a;FAST.elapsed=b;FAST.elapsed>FAST.maxElapsed&&(FAST.elapsed=FAST.maxElapsed);FAST.elapsed*=FAST.timeScale;this.step();this.currentWorldInstance.preDraw();this.currentWorldInstance.draw();FAST.visualDebug&&this.currentWorldInstance.debugDraw()},step:function(){this.currentWorldInstance.preUpdate();this.currentWorldInstance.update();this.currentWorldInstance.postUpdate()}};FAST.Obj=function(a,b){this.exists=this.visible=this.active=!0;this.fixed=!1;this.id=FAST.ID++;a=a||0;b=b||0;this.position=new FAST.V2(a,b);this.nextPosition=new FAST.V2(a,b);this.point=new FAST.V2;this.prevY=this.prevX=this.h=this.w=0;this.acceleration=new FAST.V2;this.velocity=new FAST.V2;this.nextVelocity=new FAST.V2;this.maxVelocity=new FAST.V2(25E3,25E3);this.center=new FAST.V2(this.x,this.y);this.radians=0;this.shape=null;this.fillColor=this.lineColor=16777215;this.alpha=1;this.velColor=255;
...
FAST.Island.prototype={constructor:FAST.Island,solve:function(){this.prepare();this.collectCollisions(0);this.collectCollisionsCount++;this.solveCollisions()},prepare:function(){this.collectCollisionsCount=this.collisionsResolvedThisFrame=0;this.collisions.clear();this.doubleDict=[]},compareByTime:function(a,b){return a.time-b.time},collectCollisions:function(a){var b,c,d,e;for(b=this.points.length;b--;){d=this.points[b];for(c=this.circles.length;c--;)if(e=this.circles[c],FAST.lineSegmentCircle(d.position,
d.nextPosition,e.position,e.radius,this.result),0<this.result.intersectCount){var f=this.recycleCollision();f.init(d,e,2);f.pointOfCollision.x=this.result.point1.x;f.pointOfCollision.y=this.result.point1.y;f.time=a+FAST.timeOfCollision(d.position,d.nextPosition,this.result.point1);this.addCollision(f)}for(c=this.lines.length;c--;){e=this.lines[c];var h=FAST.lineSegments(d.position,d.nextPosition,e.position,e.end,null);null!==h&&(f=this.recycleCollision(),f.init(d,e,1),f.pointOfCollision.x=h.x,f.pointOfCollision.y=
h.y,f.time=a+FAST.timeOfCollision(d.position,d.nextPosition,h),this.addCollision(f))}}},addCollision:function(a){this.collisions.insert(a);this.collisionList.add(a);a.obj1 in this.doubleDict||(this.doubleDict[a.obj1]=[]);a.obj2 in this.doubleDict[a.obj1]||(this.doubleDict[a.obj1][a.obj2]=-1)},contains:function(a,b){return-1<a.indexOf(b)},recycleCollision:function(){var a=this.collisionList.recycle();null===a&&(a=new FAST.Collision);return a},solveCollisions:function(){for(var a,b;!this.collisions.isEmpty();)if(a=
