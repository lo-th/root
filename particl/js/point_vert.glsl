...
vec3 Zoom( vec3 pos, float internTime, float bigNoize, float noizeComplexity ){

   
    float d2 = 300.0 * (internTime);

    float n = cnoise( 10.0 * pos.xy + vec2(internTime)) * bigNoize;

    float d = ( 6.0 + (internTime)) + (n); //elapsedTime * 0.5;

    //pos.x *= d;
    //pos.y *= d;
    //pos.x += n; //* perlinIntensity;
    //pos.y += n;
    pos.z += d2 + (n*100.0);//sin(phi) * sin(theta);
...
void main(){
    ...
    if( lock == 0.0 ){
        ...
        //newAlpha *= 1.0 - (time_Orbital - 0.5);
        newAlpha *= 1.0 - (time_Orbital + 0.25);
    ...
    vec3 pos_Orbital = Zoom( pos, time_Orbital, bigNoize, noizeComplexity );
