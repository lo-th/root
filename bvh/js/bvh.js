...
BVH.Reader.prototype = {
    ...
    parseNode: function ( data, NN ){
        ...
        //console.log( node.name, node.order, node.Arotation, node.Aposition );
    ...
    animate: function (){
        ...
        while(i--){
            ...
            if(node.rot.length){
                r = node.rot;
                node.quaternion.setFromEuler( euler.set( d[n+r[0]] * BVH.TO_RAD, d[n+r[1]] * BVH.TO_RAD, d[n+r[2]] * BVH.TO_RAD, node.order ) );
