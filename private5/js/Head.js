...
V.Head = function ( type, txt ) {
    ...
    this.renderer.setClearColor( 0x00d400, 1 );*/
    ...
    var v = new THREE.SphereBufferGeometry( 5, 20, 16 );
    var mb = new THREE.MeshBasicMaterial({ color:0x000000 });


    this.eyeL = new THREE.Mesh( v, mb );
    this.eyeR = new THREE.Mesh( v, mb );

    this.scene.add( this.eyeL );
    this.scene.add( this.eyeR );

    this.quad = new THREE.Mesh( new THREE.PlaneBufferGeometry( w, h ), new THREE.MeshBasicMaterial({map:type==='man'? txt.hman: txt.hwom}) );
...
V.Head.prototype = {
    ...
    update: function(x,y){
        ...
        this.eyeL.position.set( -(256 - (222+(x*8))),  -y*8, 0 );
        this.eyeR.position.set( -(256 - (290+(x*8))), -y*8, 0 );
