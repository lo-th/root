    ...
    pool = {
        ...
        sea_load: function ( url, name ) {
            ...
            l.onComplete = function( e ) {
                ...
                var i = l.meshes.length, m, list = [], o = {};
                while(i--){
                    m = l.meshes[i];
                    o[ m.name ] = m;
                    list.push( m.name );
        ...
        z_load: function ( url, name ) {
            ...
            xml.onload = function () {
                ...
                this.z_parse( name, xml.responseText );
        ...
        z_parse: function ( name, data ) {

            var ar = new Uint8Array( data.length ); 
            for (var i = 0, len = data.length; i < len; ++i){ ar[i] = data.charCodeAt(i) & 0xff; };

            lzma.decompress( ar, function o ( result ) { ar = null; results[ name ] = result; this.next(); }.bind( this ) );

        },

        toScript: function ( name ) { // main script
        ...
        toURL : function ( name ) { // for worker

            scripts[ name ] = URL.createObjectURL( new Blob( [ results[name] ], { type: 'application/javascript' }) );
            this.del( name );
            return scripts[ name ];
        ...
        delURL : function( name ){
            ...
            URL.revokeObjectURL( scripts[ name ] );
